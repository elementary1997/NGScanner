...
> Статус: приложение рабочее и выпускается (релизы по тегу `vX.Y.Z`, история —
> [CHANGELOG.md](CHANGELOG.md)). Каркасом остаются лишь отдельные места, помеченные
> `TODO(этап N)`; дорожная карта — в README.
...
app/src/main/java/ru/ngscanner/
  ...
  bluetooth/           BluetoothController — поиск и сопряжение адаптеров
  obd/                 Elm327 (драйвер), ObdParser, ObdPid (каталог PID), IsoTp,
                       DtcDatabase, FuelCalc, CustomPid (заводские PID, мода 21/22)
  llm/                 LlmProvider + ClaudeProvider, CloudRuProvider
  agent/               DiagnosticAgent (цикл), ObdTools, ObdToolExecutor
  garage/              машины, VinDecoder, VehicleCatalog, нормы, ТО (Maintenance*)
  trips/               запись поездок и «чёрный ящик»
  report/              протоколы диагностики: LLM сжимает переписку → MD/PDF
  perf/                перф-замеры (0–100, 100–0, ¼ мили)
  settings/            настройки, API-ключи, чат, избранное, расход токенов
  service/             foreground-сервис опроса, уведомления о ТО
  update/              AppUpdater — проверка версии и установка APK
  util/                Exporter (PDF/CSV/MD), ImageEncoder
  ui/                  MainScreen, MainViewModel, components/, theme/
```

Две сквозные конвенции, которым следуют все подсистемы:

- **Состояние экрана — один immutable `UiState`** в `MainViewModel` (`StateFlow`).
  Новая фича = поля в `UiState` + методы во ViewModel; своих ViewModel не заводим.
- **Репозиторий на подсистему.** Мелкие данные — JSON в `SharedPreferences`
  (`MaintenanceRepository`, `PerfRepository`, `CustomPidRepository`); объёмные —
  файлы в `filesDir` с индексом (`TripRepository`, `ReportRepository`).
...
- **Не сочинять адреса заводских PID** (мода 21/22 — ВАЗ Bosch M7.9.7 / M74, «Январь»
  и др.). Неверный адрес возвращает не ошибку, а правдоподобное НЕВЕРНОЕ число — самый
  опасный вид вранья в диагностике. Каталога таких PID в проекте нет и он не заводится
  «по памяти»: пользователь задаёт их сам (`CustomPid`), имея проверенное описание.
  Пополнять каталог можно только из проверяемого источника — см. ADR 0004.
...
- [CHANGELOG.md](CHANGELOG.md) — история версий (правится при каждом релизе)
- [docs/ecosystem.md](docs/ecosystem.md) — open-source библиотеки, базы DTC, лицензии
- [docs/adr/](docs/adr/) — архитектурные решения (реестр — в `docs/adr/README.md`)
