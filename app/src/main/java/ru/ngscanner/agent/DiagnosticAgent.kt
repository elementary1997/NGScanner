...
class DiagnosticAgent(
    ...
    companion object {
        ...
        /** Ситуативная вставка, когда адаптер не подключён — общий режим Q&A + триаж вслепую. */
        const val ADAPTER_OFFLINE_NOTE: String =
            "Адаптер ELM327 сейчас НЕ подключён. Отвечай на общие вопросы об устройстве, " +
                ...
                "вызывай OBD-инструменты (данных они не дадут).\n" +
                "Если пользователь описал СИМПТОМ, не отделывайся просьбой подключить адаптер — " +
                "дай триаж по симптому через submit_verdict: severity=НУЖНЫ ДАННЫЕ (или КРИТИЧНО, " +
                "если симптом сам по себе опасен — перегрев, стук, падение давления масла, потеря " +
                "тормозов), в causes перечисли версии по убыванию вероятности (evidence будет " +
                "помечено как неподтверждённое — это честно, данных с машины нет), а в checks — что " +
                "именно снять или измерить, чтобы отличить версии. Подключение адаптера предложи " +
                "первым пунктом checks, а не вместо ответа."
    }
}
